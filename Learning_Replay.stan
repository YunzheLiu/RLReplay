    ...
    for (t in 1:NT) { // Loop over trials
      if (c[s,t] >= 0) { // Only fit if subject entered response
        ...
        if (st[s,t] == 1) {
          ...
          Q[2, c[s,t]+1] += Phi_approx(  (alphaRIs[s] + alphaRSs[s]*( rp[s,t,3+c[s,t]]  ))  ) * ( r[s,t] - Q[2, c[s,t]+1] );
          Q[3, c[s,t]+1] += Phi_approx(  (alphaRIs[s] + alphaRSs[s]*( rp[s,t,5+c[s,t]]  ))  ) * ( r[s,t] - Q[3, c[s,t]+1] );
        }
        else if (st[s,t] == 2) {
          Q[2, c[s,t]+1] += Phi_approx(  alphaDs[s]  ) * ( r[s,t] - Q[2, c[s,t]+1] );
          Q[1, c[s,t]+1] += Phi_approx(  (alphaRIs[s] + alphaRSs[s]*( rp[s,t,1+c[s,t]]  ))  ) * ( r[s,t] - Q[1, c[s,t]+1] );
          Q[3, c[s,t]+1] += Phi_approx(  (alphaRIs[s] + alphaRSs[s]*( rp[s,t,5+c[s,t]]  ))  ) * ( r[s,t] - Q[3, c[s,t]+1] );
        }
        else if (st[s,t] == 3) {
