  ...
  for (s in 1:NS) { // Loop over subjects
    ...
    for (t in 1:NT) { // Loop over trials
      if (r[s,t] >= -2) { // Only fit if subject entered response
        ...
        Need_nl1 = nl1(st[s,t]);
        Need_nl2 = nl2(st[s,t]);
        Need_diff = Need_nl1 - Need_nl2;

        // Q-learning (non-local)
        Q[nl1(st[s,t]),c[s,t]+1] += softsign(Need_diff)   *(alphaHP*(r[s,t]-Q[nl1(st[s,t]),c[s,t]+1]))  +  softsign(Need_diff*-1)*(alphaLP*(r[s,t]-Q[nl1(st[s,t]),c[s,t]+1]));
        Q[nl2(st[s,t]),c[s,t]+1] += softsign(Need_diff*-1)*(alphaHP*(r[s,t]-Q[nl2(st[s,t]),c[s,t]+1]))  +  softsign(Need_diff)*   (alphaLP*(r[s,t]-Q[nl2(st[s,t]),c[s,t]+1]));
      }
    }
  }
}
